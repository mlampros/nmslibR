...
testthat::test_that("the KernelKnnCV_nmslib function works with default settings [ regression ]", {
  ...
  testthat::expect_true( inherits(tmp_knn, 'list') && all(names(tmp_knn) %in% c("preds", "folds")) && all(as.vector(unlist(lapply(tmp_knn, function(x) lapply(x, function(y) length(y))))) == nrow(x) / FOLDS) )
})
...
testthat::test_that("the KernelKnnCV_nmslib function works with default settings [ classification ]", {
  ...
  testthat::expect_true( inherits(tmp_knn, 'list') && all(names(tmp_knn) %in% c("preds", "folds")) &&
